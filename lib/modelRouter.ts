...
function buildSystemPrompt(analysis: BertAnalysisResult): string {
    ...
    let prompt = [
        ...
        "",
        "【患者情況分析】",
        // 修正：移除 'high' 的檢查，只檢查 'critical'，保持與三標籤 BERT 一致
        `- 風險評分：${(risk_score * 100).toFixed(0)}%`,
        `- 情緒狀態：${sentiment_score < 0.4 ? '焦慮/負面' : sentiment_score > 0.6 ? '正面/穩定' : '中性'}`,
        `- 緊急程度：${urgency_level === 'critical' ? '⚠️ 危急（建議立即就醫）' : urgency_level === 'medium' ? '⚡ 中等（建議盡快就醫）' : 'ℹ️ 低（可觀察）'}`,
    ];
...
export async function routeToModel(
    ...
    modelChoice: ModelChoice, // 可能是 'llama', 'gpt', 'auto'
    history: Message[] = []
    ...
    // --- 🚨 核心模型選擇邏輯：緊急程度優先 🚨 ---
    const urgency = (analysis.urgency_level || "low").toLowerCase();
    let finalModel: "llama" | "gpt"; // 確定最終使用的模型
    
    // 修正：只檢查 'critical'
    if (urgency === "critical") {
        // Critical 緊急程度：強制使用 GPT (用於複雜診斷和精確危機處理)
        finalModel = "gpt";
        console.log(`--- (系統強制切換模型: GPT - 緊急程度: ${urgency} - 優先級覆蓋) ---`);
    } else {
        // Low/Medium 緊急程度：
        
        // 檢查用戶是否有明確選擇 GPT (尊重用戶選擇，防止在非緊急情況下強制降級)
        if (modelChoice === "gpt") {
            finalModel = "gpt";
            console.log(`--- (使用用戶指定模型: GPT - 緊急程度: ${urgency}) ---`);
        } else {
            // auto 或 llama 選擇，都使用 LLaMA (成本效益高)
            finalModel = "llama";
            console.log(`--- (系統自動選擇模型: LLaMA - 緊急程度: ${urgency}) ---`);
        }
    }
    ...
    try {
        ...
        if (finalModel === "gpt") {
            // 使用 Groq 提供的 GPT-OSS 模型 (用於高風險/精確處理)
            const { content, debug } = await callGroqWithRetry(messages, {
        ...
        // 邏輯錯誤捕獲
        throw new Error(`模型路由邏輯錯誤，未選擇模型: ${finalModel}`);
